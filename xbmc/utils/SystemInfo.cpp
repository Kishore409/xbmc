...
#elif defined(TARGET_LINUX)
    CSysfs path("/sys/bus/soc/devices/soc0/family");
    std::string family = path.Get<std::string>();

    path = "/sys/bus/soc/devices/soc0/soc_id";
    std::string soc_id = path.Get<std::string>();
...
#elif defined(TARGET_LINUX)
    CSysfs path("/sys/bus/soc/devices/soc0/machine");
    modelName = path.Get<std::string>();
#elif defined(TARGET_WINDOWS)
    // We just don't care, might be useful on embedded
#endif
    inited = true;
